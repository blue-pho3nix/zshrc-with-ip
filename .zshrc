...
IP0=$(ip -4 addr | grep -v 127.0.0.1 | grep -v secondary | grep ppp0 | grep -Po "inet \K[\d.]+") # Get VHL IP if connected
...
if [ $IP0 ]; then
        VHL="%F{cyan}─(%B%F{green} $IP0%b%F{cyan})"
else
        VHL=""
fi

if [ $IP1 ]; then
        LOCAL="%F{cyan}─(%B%F{green} $IP1%b%F{cyan})"
else
        LOCAL=""
fi

if [ $IP2 ]; then
        VPN="%F{cyan}─(%B%F{green} $IP2%b%F{cyan})"
else
        VPN=""
fi

if [ $IP3 ]; then
        WIFI="%F{cyan}─(%B%F{red}直$IP3%b%F{cyan})"
else
        WIFI=""
fi
...
if [ "$color_prompt" = yes ]; then
    ...
    RPROMPT=$'%F{reset}'$TIME'%(?.. %? %F{red}%B⨯%b%F{reset})%(1j. %j %F{yellow}%B⚙%b%F{reset}.)'
...
alias l='ls -CF'
alias c='clear'
alias up='sudo apt-get update && sudo apt-get full-upgrade -y && sudo apt autoclean && sudo apt autoremove -y'
